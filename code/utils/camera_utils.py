...
def loadCam(conf, id, cam_info, resolution_scale):
    orig_w, orig_h = cam_info.image.size
    resolution = conf.get_int('gs.model.resolution')
    data_device = conf.get_string('gs.model.data_device')
    ...
    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T, 
                  ...
                  image=gt_image, normal=gt_normal, edge=edge, gt_alpha_mask=loaded_mask,
                  image_name=cam_info.image_name, uid=id, data_device=data_device)

def cameraList_from_camInfos(cam_infos, resolution_scale, conf):
    camera_list = []

    for id, c in enumerate(cam_infos):
        camera_list.append(loadCam(conf, id, c, resolution_scale))
